...
import {draftStore,outbox,receipts} from './drafts.js?v=2.2.1';
import {localRecords,localFile,validateRecord,uid,metadata} from './store.js?v=2.2.1';
import {sha256} from './crypto.js?v=2.2.1';
import {esc,icon,toast,bytes,localDate,download} from './ui.js?v=2.2.1';
const {values:categories,defaultValue:defaultCategory,normalize:normalizeCategory,isValid:isCategory}=globalThis.ActivityCategories;
...
async function parseExcel(){
 ...
 excelOptions.category=$('#excel-category').value;excelOptions.year=$('#excel-year').value;excelOptions.includeHidden=$('#excel-hidden').checked;
 try{const buffer=await excelFile.arrayBuffer();const w=new Worker(new URL('./excel-worker.js?v=2.2.1',import.meta.url));worker=w;
...
async function rowToRecord(r){
 ...
 return validateRecord({id:uid(),title:r.title.trim(),date,time:'',work:r.work.trim(),people,category:r.category,location:r.location||'',notes:[r.notes,`导入来源：${excelFile?.name||'历史表格'} / ${r.sheet} / 第 ${r.row} 行；原日期：${r.originalDate||r.date}`].filter(Boolean).join('\n'),attachments:[],starred:false,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),importFingerprint:await sha256(signature)});
}
...
async function confirmExcelImport(){
 if(importing)return;
 const selected=excelRows.filter(r=>r.include&&!r.imported);if(!selected.length||selected.length>500){toast('请选中 1–500 条记录。',true);return;}if(selected.some(r=>!validRow(r))){toast('仍有日期、活动类型或必填字段无效，请修正后再导入。',true);return;}if(!$('#excel-confirm').checked){toast('请先勾选“我已核对”，尤其确认年份与人员分工。',true);return;}
