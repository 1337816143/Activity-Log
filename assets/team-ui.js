import {CATEGORY_OPTIONS} from './categories.js?v=2.2.1';
...
async function rowToRecord(r){
 ...
 return validateRecord({id:uid(),title:r.title.trim(),date,time:'',work:r.work.trim(),people,category:r.category?.trim()||'其他',location:r.location||'',notes:[r.notes,`导入来源：${excelFile?.name||'历史表格'} / ${r.sheet} / 第 ${r.row} 行；原日期：${r.originalDate||r.date}`].filter(Boolean).join('\n'),attachments:[],starred:false,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),importFingerprint:await sha256(signature)});
}
