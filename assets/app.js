...
class URLPool {
  ...
  async get(a,original=false){const key=a.id+(original?':original':':thumb');if(this.cache.has(key))return this.cache.get(key);const generation=this.generation;
    const promise=(async()=>{const f=demoFiles.get(a.id)||await getFile(a.id,!original);if(!f?.blob)throw new Error('原附件不存在，请从完整备份恢复。');const url=URL.createObjectURL(original?f.blob:(f.thumbnail||f.blob));if(this.generation!==generation){URL.revokeObjectURL(url);throw new Error('已切换预览');}return url;})();
...
let stagedURLs=[];
let workingId=null,draftCloudRevision=null,draftTimer=0,cloudTimer=0,draftWrite=Promise.resolve(),cloudWrite=null;
...
async function removeRecord(r){
 if(state.demo){toast('示例无需删除。');return;}if(!team.connected||!r._team){toast('请先连接并迁移旧记录，再进行受保护的删除。');showTeam();return;}
 const password=await askAdmin(`将「${r.title}」移入回收站`);if(password===null)return;
 const button=$('#delete-record');button.disabled=true;
 try{await deleteRecord(r,password);await refresh();changed();toast('活动已移入回收站；附件与历史仍保留。');}catch(e){safeError(e);}finally{if(button.isConnected)button.disabled=false;}
}
...
async function persistWorking({cloud=false,quiet=false}={}){
 if(!workingId||!$('#editor').open)return null;clearTimeout(draftTimer);
 const snapshot=workingSnapshot();draftWrite=draftWrite.catch(()=>{}).then(()=>draftStore.save(snapshot));await draftWrite;
 if(!saving)$('#form-status').textContent=`已暂存于本机 ${new Date().toLocaleTimeString('zh-CN',{hour12:false})} · ${team.connected?'等待同步':'共享服务未连接'}`;
 if(cloud){await syncWorking(snapshot);if(!quiet)toast('草稿和附件已暂存到团队。');}
 else if(!quiet)toast('已暂存信息和原附件，可关闭后从草稿箱继续。');
 return snapshot;
}
function scheduleWorking(){
 dirty=true;clearTimeout(draftTimer);clearTimeout(cloudTimer);
 draftTimer=setTimeout(()=>persistWorking({quiet:true}).catch(safeError),600);
 if(team.connected)cloudTimer=setTimeout(()=>{if(!saving&&!processing&&$('#editor').open)persistWorking({cloud:true,quiet:true}).catch(safeError);},30000);
}
async function syncWorking(snapshot){
 if(!team.connected)throw new Error('本机暂存已完成；共享服务恢复后才能同步团队草稿。');
 if(cloudWrite)await cloudWrite;
 const run=async()=>{
  const latest=await draftStore.get(snapshot.id);if(!latest)return;
  const newFiles=[];for(const a of latest.staged){if(a.blob)newFiles.push(a);else if(!team.manifest.files?.[a.id]){const f=await getFile(a.id);if(!f?.blob)throw new Error('草稿缺少原附件，请重新选择文件。');newFiles.push({...a,...f});}}
  const record={id:latest.id,title:latest.fields.title,fields:latest.fields,attachments:latest.staged.map(metadata),editor:{editing:latest.editing,removed:latest.removed},_revision:latest.cloudRevision??null,updatedAt:latest.updatedAt};
  const result=await team.saveRecord(record,newFiles,[],{draft:true});
  const current=await draftStore.get(latest.id);if(current)await draftStore.save({...current,cloudRevision:result.revision,cloudAt:new Date().toISOString()});
  if(workingId===latest.id){draftCloudRevision=result.revision;if(!saving)$('#form-status').textContent='草稿已在共享库保存 · 后续修改仍会自动暂存';}
 };
 cloudWrite=run();try{await cloudWrite;}finally{cloudWrite=null;}
}
async function resumeWorking(d){
...
async function closeEditor(){
 if(saving||processing){toast('原信息已暂存，正在完成附件处理或保存，请稍后关闭。');return;}
 try{clearTimeout(cloudTimer);if(dirty||staged.some(a=>!a.existing)){const snap=await persistWorking({quiet:true});if(team.connected&&snap)syncWorking(snap).catch(safeError);}
  $('#editor').close();releaseStaged();editing=null;staged=[];dirty=false;workingId=null;
 }catch(e){safeError(e);}
}
...
async function stageFiles(files){
 if(saving)return;const list=[...files];if(!list.length)return;
 if(staged.length+list.length>100){toast('每条活动最多 100 个附件。',true);return;}
 if(list.reduce((s,f)=>s+f.size,0)>200*1024*1024){toast('一次添加不能超过 200 MB，请分批添加。',true);return;}
 if(list.some(f=>f.size>MAX_FILE)){toast('有单个附件超过 50 MB，未添加本批文件。',true);return;}
 const thisDraft=workingId;
 const pending=list.map(file=>({file,data:{id:uid(),name:file.name.slice(0,255),type:mimeFor(file),size:file.size,blob:file.slice(0,file.size,mimeFor(file)),thumbnail:null}}));
 staged.push(...pending.map(p=>p.data));dirty=true;processing++;$('#save-record').disabled=true;renderStaged();
 try{
  // Persist raw bytes before generating thumbnails or attempting network upload.
  await persistWorking({quiet:true});
  for(const {file,data} of pending){const prepared=await prepareFile(file);data.thumbnail=prepared.thumbnail;if(workingId===thisDraft&&$('#editor').open){renderStaged();await persistWorking({quiet:true});}}
 }catch(e){safeError(e);}finally{processing--;if($('#editor').open&&workingId===thisDraft){$('#save-record').disabled=processing>0;$('#editor-files').value='';scheduleWorking();}}
}
async function submitRecord(e){
 ...
 try{await persistWorking({quiet:true});}catch(e){safeError(e);return;}
 if(!team.connected){toast('内容已暂存，请在共享服务恢复后正式保存。');showTeam();return;}
 clearTimeout(cloudTimer);clearTimeout(draftTimer);
 let password='';if(removed.length){password=await askAdmin('从已保存的活动中移除附件');if(password===null)return;}
 ...
 try{
  ...
  $('#editor').close();releaseStaged();workingId=null;await refresh();changed();toast('活动与附件已在共享库保存，团队成员可刷新查看。');
 }catch(err){safeError(err);$('#form-status').textContent='尚未确认保存；本机草稿与附件仍保留，请先查看同步状态。';}
 finally{password='';saving=false;lockEditor(false);}
}
async function quickUpload(files){
 const r=currentRecord();if(!r||state.demo){toast('请先选择一条真实活动记录。');return;}
 const list=[...files];if(!list.length)return;
 openEditor(r);await stageFiles(list);toast('附件已暂存到编辑草稿，请核对后点击保存，同步到团队。');$('#quick-files').value='';
}
...
async function renderPreview(){
 ...
 if(isImage(a)&&!zoomed){body.innerHTML='<div id="popup-deck"></div>';popupDeck=new PhotoDeck($('#popup-deck'),previewItems,a=>popupURLs.get(a,false),{initial:previewIndex,onChange:updatePopup,onOpen:()=>{zoomed=true;renderPreview();}});popupDeck.root.focus({preventScroll:true});}
 else {
  ...
  try{
   ...
   else if(a.type==='application/pdf'||/\.pdf$/i.test(a.name)){body.innerHTML=`<iframe class="file-preview-frame" sandbox="allow-same-origin allow-downloads" referrerpolicy="no-referrer" title="PDF 预览：${esc(a.name)}" src="${url}"></iframe>`;$('#popup-meta').textContent='PDF 预览依赖浏览器支持；空白时请下载原文件';}
...
$('#start-own').onclick=()=>{ownMode();toast('已退出示例。可先填写并暂存，无需登录，直接填写并保存即可。');};
...
$('#close-editor').onclick=closeEditor;$('#cancel-editor').onclick=closeEditor;$('#editor').addEventListener('cancel',e=>{e.preventDefault();closeEditor();});$('#record-form').onsubmit=submitRecord;$('#record-form').addEventListener('input',scheduleWorking);$('#record-form').addEventListener('change',scheduleWorking);
$('#save-draft').onclick=()=>persistWorking({quiet:false}).catch(safeError);$('#save-cloud-draft').onclick=()=>persistWorking({cloud:true}).catch(safeError);
$('#editor-files').onchange=e=>stageFiles(e.target.files);
$('#staged-files').onclick=e=>{const b=e.target.closest('[data-remove-staged]');if(!b||saving||processing)return;const i=Number(b.dataset.removeStaged),a=staged[i];if(a.existing)removed.push(a.id);staged.splice(i,1);dirty=true;renderStaged();scheduleWorking();};
...
initializeTeamUI({onOpenDraft:resumeWorking,onRefresh:async()=>{if(team.connected){state.demo=false;prefs.set('mode','own');}detailKey='';await refresh();}});
team.addEventListener('changed',()=>{if(!$('#editor').open)refresh().catch(safeError);});
team.addEventListener('status',e=>{if(saving)$('#form-status').textContent=e.detail.text;});

window.addEventListener('focus',()=>{if(team.connected&&!team.busy)team.refresh(true).then(()=>refresh()).catch(safeError);});

(async()=>{
 try{state.ready=true;await team.connect();state.records=await allRecords();state.demo=prefs.get('mode')==='demo';options();render();}
 catch(e){state.ready=true;state.records=[];render();$('#records').innerHTML=empty('暂时连接不上共享服务','可以继续新建并暂存草稿，网络恢复后再保存。点击「共享与备份」重新连接。');safeError(e);}
})();
