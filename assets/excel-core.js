 ...
 function analyze(sheets,options={}){
  ...
  for(const sheet of sheets){if(sheet.hidden&&!options.includeHidden){skipped.push({sheet:sheet.name,reason:'隐藏工作表，默认不导入'});continue;}
   ...
   function add(vals,row,notes=[]){const title=text(vals.title),work=text(vals.work),people=text(vals.people),originalDate=text(vals.date);if(![title,work,people,originalDate].some(Boolean))return;
    const parsed=parseDate(vals.date,options.year);const warnings=[...notes,...(parsed.warning?[parsed.warning]:[])];for(const [name,v] of [['活动名称',title],['具体工作',work],['人员',people]])if(!v)warnings.push(name+'缺失');const cat=text(vals.category);if(cat&&!types()?.normalize(cat))warnings.push('活动类型不在新七项分类中，请选择；原类型：'+cat);records.push({sheet:sheet.name,row,title,work,people,date:parsed.date,originalDate,warnings,include:!!(title&&work&&people&&parsed.date&&(!cat||types()?.normalize(cat))),category:types()?.normalize(cat)||(cat?'':(types()?.normalize(options.category)||'部门活动')),originalCategory:cat,location:'',notes:''});
   }
